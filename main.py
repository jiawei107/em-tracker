...
Track manuscript status from environment variables
"""
...
try:
    ...
    print(f"[Error] config.py not found: {e}")
    exit(1)
...
def send_to_serverchan(title: str, content: str) -> bool:
    """Send message to WeChat via ServerChan API"""
    if not SERVERCHAN_SENDKEY:
        logger.warning("ServerChan SendKey not configured, skipping push")
        return False
    ...
    try:
        ...
        if result.get('code') == 0:
            ...
            logger.error(f"WeChat push failed: {result.get('message', 'Unknown error')}")
            return False
...
def perform_login(account: dict) -> Union[requests.Session, None]:
    """Execute login and return authenticated session"""
    ...
    logger.info(f"Logging in to '{account['journal_full_name']}'...")
    ...
    for attempt in range(DEFAULT_RETRY_COUNT):
        try:
            ...
            if LOGIN_SUCCESS_FLAG in response.text:
                logger.info(f"Login successful! Account: {account['username']}")
                return session
            else:
                logger.error(f"Login failed. Please check account: {account['username']}")
                return None
        ...
        if attempt < DEFAULT_RETRY_COUNT - 1:
            logger.info(f"{RETRY_DELAY_SECONDS} seconds before retry...")
            time.sleep(RETRY_DELAY_SECONDS)

    logger.error(f"All login attempts failed: {account['username']}")
    return None
...
def fetch_manuscript_details(session: requests.Session, detail_url: str, referer_url: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug(f"Successfully fetched {len(manuscript_list)} manuscript records")
...
def fetch_submission_overview(session: requests.Session, account: dict) -> Union[List[Dict[str, Any]], None]:
    ...
    logger.info("Fetching manuscript list...")
    ...
    try:
        ...
        logger.error(f"Failed to fetch main menu: {e}")
        return None
    ...
    try:
        ...
        logger.info(f"Found {len(all_manuscripts)} manuscript records")
...
def find_value_by_partial_key(data: Dict[str, str], key_parts: List[str]) -> str:
    """Match value by partial keywords"""
...
def main():
    """Main function"""
    logger.info("=" * 60)
    logger.info("Journal Manuscript Tracker")
    logger.info("=" * 60)
    ...
    for account in ACCOUNTS:
        if 'your_username' in account.get('username', '').lower():
            logger.warning(f"Skipping unconfigured account: {account.get('username', '')}")
            continue

        logger.info("=" * 60)
        logger.info(f"Journal: {account['journal_full_name']} ({account['journal_short_name']})")
        ...
        if not manuscripts:
            logger.info("No manuscript records.")
            continue
        ...
        for manuscript in manuscripts:
            ...
            logger.info(f"Manuscript: {title[:50]}...")
            logger.info(f"  Number: {ms_number}")
            logger.info(f"  Status: {status}")
            logger.info(f"  Status Date: {status_date}")

            wechat_content += f"Manuscript: **{title[:40]}...**\n"
            wechat_content += f"• Number: {ms_number}\n"
            wechat_content += f"• Status: {status}\n"
            wechat_content += f"• Date: {status_date}\n\n"
    ...
    if all_results:
        ...
        for result in all_results:
            logger.info(
                ...
                f"Number: {result['Manuscript Number']} | "
                f"Status: {result['Current Status']} | "
                f"Status Date: {result['Status Date']}"
            )

        logger.info(f"Processed {len(all_results)} manuscript records")

        wechat_title = f"Manuscript Tracking ({datetime.now().strftime('%Y-%m-%d %H:%M')})"
        if wechat_content.strip():
            send_to_serverchan(wechat_title, wechat_content)
    else:
        logger.warning("No manuscript information retrieved.")
